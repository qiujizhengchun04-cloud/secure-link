...
const T = {
  en: {
    ...
    copyDone: '✅ Copy done!',
    copy: '📋 Copy',
    doneMsg: 'Done message (optional)',
    donePlaceholder: 'e.g. Welcome!'
  },
  jp: {
    title: 'コマンド + リンク生成',
    prompt: 'コマンド入力',
...
#bg-logo {
  ...
  pointer-events:none;
  z-index:0;
  text-shadow:0 0 80px rgba(100,200,255,0.05);
  animation:glitchLogo 8s infinite linear, pulseLogo 4s ease-in-out infinite;
  font-family:'Impact','Arial Black',sans-serif;
  white-space:nowrap;
}
@keyframes glitchLogo {
  0%,90%,100% { transform:translate(-50%,-50%) rotate(-15deg) scale(1.2) skewX(0deg); opacity:1; }
  92% { transform:translate(-50%,-50%) rotate(-15deg) scale(1.25) skewX(5deg); opacity:0.7; text-shadow:0 0 100px rgba(0,255,255,0.3); }
  94% { transform:translate(-50%,-50%) rotate(-15deg) scale(1.15) skewX(-5deg); opacity:0.9; text-shadow:0 0 60px rgba(255,0,255,0.2); }
  96% { transform:translate(-50%,-50%) rotate(-15deg) scale(1.2) skewX(3deg); opacity:1; text-shadow:0 0 100px rgba(100,200,255,0.15); }
}
@keyframes pulseLogo {
  0%,100% { opacity:0.03; }
  50% { opacity:0.08; }
...
#cmd-output::-webkit-scrollbar-thumb { background:#555; border-radius:3px; }
#cmd-input-line {
  display:flex;
  align-items:center;
...
#menu-window .btn-link:disabled { opacity:0.4; cursor:not-allowed; }
...
<div id="menu-window" class="window">
  ...
  <div class="custom-label" id="done-label">Done message (optional)</div>
  <input type="text" class="custom-input" id="done-text" placeholder="e.g. Welcome!" spellcheck="false">
    ...
    if (e.key === 'Enter') {
      ...
      if (cmd === '/jp') {
        fetch('/lang/jp').then(function() {
          ...
          document.getElementById('done-label').textContent = '完了メッセージ（任意）';
      ...
      if (cmd === '/en') {
        fetch('/lang/en').then(function() {
          ...
          document.getElementById('done-label').textContent = 'Done message (optional)';
